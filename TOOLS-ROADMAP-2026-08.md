...
| 3 | **Xtream ⇄ M3U URL generator** (client-side) | iptvesp | The "listas iptv" market (1,900/mo cluster); Spanish playlist intent | "convertir xtream a m3u", "generar lista m3u", "xtream codes a m3u" | ✅ shipped 31 Jul — audit-verified (client-side, SoftwareApplication schema, privacy note, sitemap+internal links ✓). Nits: add FAQPage JSON-LD to the existing FAQ; fix 1 new redirect-link |
| 4 | **IPTV bandwidth calculator** ("Quel débit pour l'IPTV ?" — channels/quality → Mbps) | primeiptv | Upgrades its existing /guides/iptv-4k-debit-internet into an interactive asset | "debit iptv", "vitesse internet iptv", 4k requirements | ✅ shipped 1 Aug — audit-verified (title ≤60, FAQPage+SoftwareApplication schema, guide cross-links, sitemap ✓) |
